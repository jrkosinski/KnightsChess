...
namespace KnightsChess
{
    ...
    class Solution
    {
        ...
        private int CalculateEndGame(Point current, Point destination, List<Point> lastNMoves, int numberOfMoves)
        {
            ...
            //we will save points we've already looked at, here, so that we don't need to look at them more than once 
            List<Point> savedPoints = new List<Point>();
            ...
            while (true)
            {
                ...
                for (int i = 0; i < pointsToCheckFrom.Count; i++)
                {
                    ...
                    foreach (var move in allMoves)
                    {
                        ...
                        if (!savedPoints.Contains(move))
                        {
                            savedPoints.Add(move);
